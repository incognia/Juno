...
I teach in a STEM workshop for middle school students (ages 12 to 15). I need to set up a programming laboratory with JupyterLab. I use a Docker host to deploy multiple containers, launching them with Docker Compose. The compose.yaml file is generated by a Python script that reads a plain text file containing the list of students and creates N containers based on the content of that list.

There are additional scripts for managing container rebuilding, host cleanup, volume deletion, and distributing tasks in Jupyter Notebook (.ipynb) format to all containers.
